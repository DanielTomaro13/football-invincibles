 ...
 * The same bar lives at the top of AFL 23-0 and NRL 24-0 so all three sites
 * point at one another. Self-contained inline styles — no global CSS needed.
 */
const SITES = [
...
export default function SisterSites({ active }: { active: "afl" | "nrl" | "football" }) {
  return (
    ...
    >
      <span style={{ color: "var(--muted,#93a0bd)", marginRight: 2, fontWeight: 700, fontSize: ".7rem" }}>
        THE 0 SERIES ·
      </span>
      {SITES.map((s) =>
        s.key === active ? (
          <span
            ...
            style={{
              ...
              background: "var(--panel-2,#1d2740)",
              border: "1px solid var(--border,#26314d)",
              fontWeight: 600,
            }}
          >
            {s.label}
          ...
          <a
            key={s.key}
            href={s.href}
            style={{ whiteSpace: "nowrap", padding: "3px 9px", borderRadius: 999, color: "var(--muted,#93a0bd)", fontWeight: 600 }}
          >
            {s.label}
          </a>
        )
